...
print(1900 in cennik.values()) # True
print()
...
# Tak było do Pythona 3.10. Od Python 3.11 można już pisać dwa razy ten sam rodzaj cudzysłowu:
print(f'czajnik kosztuje {cennik['czajnik']}')
...
# aby poniższą operację wykonać dwa razy: raz dla pralki, która istnieje, a drugi raz dla pomidora, który nie istnieje w cenniku
for towar in 'pralka', 'pomidor':
    if towar in cennik:
        print(f'Towar {towar} kosztuje {cennik[towar]}')
    else:
        print(f'Towaru {towar} nie ma w cenniku')
print()

# Inny zapis, podejście "ofensywne" - próbujemy odczytać, a w razie błędu obsługujemy wyjątek
for towar in 'pralka', 'pomidor':
    try:
...
# Najprościej operatorem del:
print(cennik)
del cennik['kuchenka']
print(cennik)

# Wyczyszczenie całej zawartości to operacja clear():
# cennik.clear()
...
posortowane_dni_tygodnia = sorted(dni_tygodnia.values())
