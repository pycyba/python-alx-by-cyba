...
# list comprehension - jednolinijkowe utworzenie nowej listy na podstawie innej kolekcji (lub innego źródła danych)
...
dlugosci_nazw3 = [len('Warszawa'), len('Kraków'), len('Łódź'), len('Wrocław'), len('Poznań')]
print(dlugosci_nazw3)
...
# mi się to kojarzy z SELECT upper(miasto) FROM miasta WHERE miasto.startswith('W')
print(duze_litery)
...
suma_nieparzystych = sum([2*x+1 for x in range(9)])
...
# lista list:
tabliczka2d = [[w*k for k in range(1,11)] for w in range(1,11)]
print(tabliczka2d)
print()
...
generator = (z*z for z in range(-5, 6))
...
# Generator da (wygeneruje) wartości dopiero wtedy, gdy wywołamy na nim funkcję next()
# a pośrednio stanie się tak, gdy przekażemy ten generator do pętli for.
