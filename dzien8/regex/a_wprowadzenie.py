...
# Gdy w wyrażeniu regularnym tworzone są grupy za pomocą (nawiasów), to `findall` zwraca listę krotek,
# które pasują do poszczególnych grup, a nie listę dopasowanych tekstów.
tekst = '12.01.2008 lalalala 11 to nie to 3.4.2005 14 lelele 07.07.1997'
lista = re.findall(r'([0-9]{1,2})\.([0-9]{1,2})\.([0-9]{4})', tekst)
print('lista wynikowa 1:', lista)

# Gdy grupa w nawiasach jest tylko jedna, to wtedy wynikiem jest lista tekstów
# pasujących do grupy nr 1
lista = re.findall(r'[0-9]{1,2}\.([0-9]{1,2})\.[0-9]{4}', tekst)
print('lista wynikowa 2:', lista)

"""
Oznaczanie liczby powtórzeń
? → 0 lub 1
* → 0 lub więcej
+ → 1 lub więcej
{n} → konkretna liczba
...
Przykładowo
ab+  → jedno a i 1 lub więcej b
(ab)* → zero lub więcej powtórzeń ciągu ab, np. ababab
[ab]* → zero lub więcej liter a b dowolnie wymieszanych
[a-z]+ → jedna lub więcej dowolna mała litera
"""
