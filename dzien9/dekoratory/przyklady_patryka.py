...
def nie_zero(funkcja):
    def wrapper(liczba, *args, **kwargs):
        ...
        return funkcja(liczba, *args, **kwargs)
    return wrapper
...
# dekorator sparametryzowany :-)
# należy napisać funkcję, która (po uzyskaniu parametru) zwraca dekorator
# inaczej: należy napisać funkcję, która w wynik zwraca funkcję, która w wyniku zwraca funkcję
def sprawdz_wynik(predykat):
    def sprawdz_wynik_dekorator(funkcja):
        def wrapper(*args, **kwargs):
            wynik = funkcja(*args, **kwargs)
            if predykat(wynik):
                return wynik
            raise ValueError(f'Błędny wynik ({wynik})')
        return wrapper
    return sprawdz_wynik_dekorator

@sprawdz_wynik(lambda wynik: wynik <= 100)
def kwadrat(x):
    return x*x
